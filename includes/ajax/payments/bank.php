...
try {
  ...
  switch ($_POST['handle']) {
    ...
    case 'subscribe':
      ...
      // process
      $db->query(sprintf("INSERT INTO bank_transfers (user_id, handle, node_id, node_type, price, bank_receipt, time) VALUES (%s, 'subscribe', %s, %s, %s, %s)", secure($user->_data['user_id'], 'int'), secure($_POST['id'], 'int'), secure($_POST['node']), secure($_POST['price']), secure($_POST['bank_receipt']), secure($date))) or _error("SQL_ERROR_THROWEN");
